...
point *generatePower(int n,
                     double p);  // points selon loi puissance sur un disque
point *generateConvex(int n);    // n points au hasard en position convexe
point *generateGrid(int *n, int p,
                    int q);  // points sur une grille régulière pxq
...
void drawPartialTour(point *V, int n, int *P,
                     int k);  // affiche une tournée partielle avec k points
...
void drawGraph(point *V, int n, int *P,
               graph G);  // affiche graphe, arbre et tournée
...
typedef struct {
  ...
  int **value;     // valuation des cases: value[x][y], 0<=x<X, 0<=y<Y
...
// les variables initialisées ailleurs (dans tools.c). Suivant la même
// logique on pourrait ajouter des "extern" devant toutes les
// fonctions des .h, mais cela semble inutile.

extern int width,
    height;           // taille de la fenêtre, mise à jour si redimensionnée
...
//  u -> recentrage du nuage de points au centre de la fenêtre
