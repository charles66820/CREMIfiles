...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0 || rank == 1) {
    if (rank == 0) {
      ...
      printTab(tab, size);
      MPI_Isend(&tab, size, MPI_FLOAT, 1, 100, MPI_COMM_WORLD,
                &reqSend);
      MPI_Irecv(&rTab, size, MPI_FLOAT, 1, 101, MPI_COMM_WORLD,
                &reqRecv);
      ...
      for (size_t i = 0; i < size; i++) tab[i] = 1;
      MPI_Irecv(&rTab, size, MPI_FLOAT, 0, 100, MPI_COMM_WORLD,
                &reqRecv);
      MPI_Wait(&reqRecv, &statusRecv);
      printf("P%d receive : ", rank);
      printTab(rTab, size);
      ...
      printf("P%d send : ", rank);
      printTab(tab, size);
      MPI_Isend(&tab, size, MPI_FLOAT, 0, 101, MPI_COMM_WORLD,
                &reqSend);
      MPI_Wait(&reqSend, &statusSend);
    }
  }
