 ...
 */
Expression *ConstruireNoeud(expr_t type, Expression *g, Expression *d,
                            char **args) {
  Expression *e;
 ...
 */
void EndOfFile(void) { exit(0); } /* EndOfFile */
...
void expression_free(Expression *e) {
  ...
  if (e->arguments != NULL) {
    for (int i = 0; e->arguments[i] != NULL; i++) free(e->arguments[i]);
    free(e->arguments);
  }
