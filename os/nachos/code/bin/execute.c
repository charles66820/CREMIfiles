...
runprogram(startpc, argc, argv) int startpc, argc;
char *argv[];
{
  int aci, ai, j;
    ...
    if (instr != 0) /* eliminate no-ops */
    {
      switch ((instr >> 26) & 0x0000003f) {
        ...
        case I_BCOND: {
          ...
          {
            case I_BLTZ:
              if (Reg[rs(instr)] < 0) npc = xpc + 4 + (immed(instr) << 2);
              break;
            case I_BGEZ:
              if (Reg[rs(instr)] >= 0) npc = xpc + 4 + (immed(instr) << 2);
              break;
...
dump_reg() {
  ...
  printf(" 0:");
  for (j = 0; j < 8; ++j) printf(" %08x", Reg[j]);
  printf("\n");
  printf(" 8:");
  for (; j < 16; ++j) printf(" %08x", Reg[j]);
  printf("\n");
  printf("16:");
  for (; j < 24; ++j) printf(" %08x", Reg[j]);
  printf("\n");
  printf("24:");
  for (; j < 32; ++j) printf(" %08x", Reg[j]);
  printf("\n");
}
...
void henters(n, hist) int n, hist;
{
  if (0 <= n && n < NNN)
    ++hists[hist][n];
  else
...
patable(tab) int tab[33][33];
{
  ...
  printf("  |");
  for (j = 0; j < 33; ++j) printf(fmt, j);
  putchar('\n');
  printf("  |");
  for (j = 0; j < 33; ++j) printf(fmt2);
  putchar('\n');
  for (i = 0; i < 33; ++i) {
    printf("%2d|", i);
    for (j = 0; j < 33; ++j) printf(fmt, tab[i][j]);
    putchar('\n');
  }
}
...
henter(n) int n;
{
  if (0 <= n && n < NNNN) ++hist[n];
}

printhist() {
  int i;

  for (i = 0; i < NNNN; ++i) printf("%d %d\n", i, hist[i]);
}
