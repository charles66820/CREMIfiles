...
char *symbol_type[] = {"Nil",      "Global",  "Static",     "Param",
                       "Local",    "Label",   "Proc",       "Block",
...
char *argv[];
{
  ...
  if (!read_struct(f, filehdr) || !read_struct(f, aouthdr) ||
  ...
  for (i = 0; i < filehdr.f_nscns; ++i) {
    ...
    if (scnhdr[i].s_size > MAXDATA * sizeof(long) && scnhdr[i].s_scnptr != 0 ||
...
print_section(i) int i;
{
  ...
  printf("Section: %s\t%d/%d\n", scnhdr[i].s_name, scnhdr[i].s_size,
         section[i].relocs);
...
char *section_name[] = {"(null)", ".text", ".rdata", ".data", ".sdata",
                        ".sbss",  ".bss",  ".init",  ".lit8", ".lit4"};

char *reloc_type[] = {"abs",  "16",   "32",     "26",
