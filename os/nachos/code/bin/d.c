...
char *regstrings[] = {"0",   "r1",  "r2",  "r3",  "r4",  "r5",  "r6",  "r7",
...
dump_ascii(instruction, pc) int instruction, pc;
{
  int addr;
  char *s;
  int opcode;
  ...
  if (instruction == I_NOP) {
    ...
    switch (opcode) {
      ...
      case I_SRL:
      case I_SRA:
        printf("%s,%s,0x%x", R(rd(instruction)), R(rt(instruction)),
               shamt(instruction));
        break;
      ...
      case I_SRLV:
      case I_SRAV:
        printf("%s,%s,%s", R(rd(instruction)), R(rt(instruction)),
               R(rs(instruction)));
        break;
      ...
      case I_SLT:
      case I_SLTU:
        printf("%s,%s,%s", R(rd(instruction)), R(rs(instruction)),
               R(rt(instruction)));
        break;
    }
    ...
    switch (opcode) {
      ...
      case I_BEQ:
      case I_BNE:
        printf("%s,%s,%08x", R(rt(instruction)), R(rs(instruction)),
      ...
      case I_ORI:
      case I_XORI:
        printf("%s,%s,0x%x", R(rt(instruction)), R(rs(instruction)),
               immed(instruction));
        break;
      ...
      case I_SWC2:
      case I_SWC3:
        printf("%s,0x%x(%s)", R(rt(instruction)), immed(instruction),
               R(rs(instruction)));
        break;
    }
  }
