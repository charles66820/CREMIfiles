...
void Machine::Run() {
  ...
  if (DebugIsEnabled('m'))
    ...
    printf("Starting thread \"%s\" at %d and %d at time %lld\n",
           currentThread->getName(), machine->ReadRegister(PCReg),
           machine->ReadRegister(NextPCReg), stats->totalTicks);
  // End of correction
...
void Machine::OneInstruction(Instruction *instr) {
  ...
  switch (instr->opCode) {
    ...
    case OP_LBU:
      ...
      if (!machine->ReadMem(tmp, 1, &value)) return;
    ...
    case OP_LHU:
      ...
      if (!machine->ReadMem(tmp, 2, &value)) return;
    ...
    case OP_LW:
      ...
      if (!machine->ReadMem(tmp, 4, &value)) return;
      nextLoadReg = instr->rt;
      nextLoadValue = value;
      break;
    ...
    case OP_LWR:
      ...
      if (!machine->ReadMem(tmp & ~0x3, 4, &value)) return;
    ...
    case OP_MULT:
      Mult(registers[instr->rs], registers[instr->rt], TRUE, &registers[HiReg],
           &registers[LoReg]);
      break;

    case OP_MULTU:
      Mult(registers[instr->rs], registers[instr->rt], FALSE, &registers[HiReg],
           &registers[LoReg]);
      break;
    ...
    case OP_SB:
      if (!machine->WriteMem((unsigned)(registers[instr->rs] + instr->extra), 1,
                             registers[instr->rt]))
        return;
      break;

    case OP_SH:
      if (!machine->WriteMem((unsigned)(registers[instr->rs] + instr->extra), 2,
                             registers[instr->rt]))
        return;
      break;
    ...
    case OP_SLL:
      registers[instr->rd] =
          (int)(((unsigned)registers[instr->rt]) << instr->extra);
      break;
    ...
    case OP_SW:
      if (!machine->WriteMem((unsigned)(registers[instr->rs] + instr->extra), 4,
                             registers[instr->rt]))
        return;
      break;
    ...
    case OP_SWR:
      ...
      if (!machine->WriteMem((tmp & ~0x3), 4, value)) return;
      break;
