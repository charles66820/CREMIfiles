...
static const char *exceptionNames[] = {"no exception",
                                       "syscall",
...
void Machine::DumpState() {
  ...
  for (i = 0; i < NumGPRegs; i++) switch (i) {
      case StackReg:
...
void Machine::DumpReg(FILE *output, int val, const char *name,
                      const char *color, int ptr_x, int ptr_y,
                      unsigned virtual_x, unsigned y, unsigned blocksize) {
  unsigned page = val / PageSize;
  unsigned offset = val % PageSize;

  fprintf(output,
          "<text x=\"%d\" y=\"%u\" stroke=\"%s\" fill=\"%s\" "
          "font-size=\"%u\">%s</text>\n",
          ptr_x, y - page * blocksize, color, color, blocksize, name);
  fprintf(output,
          "<line x1=\"%d\" y1=\"%u\" x2=\"%u\" y2=\"%u\" "
          "stroke=\"#808080\" stroke-width=\"5\"/>\n",
          ptr_x + 3 * blocksize / 2, ptr_y - page * blocksize - blocksize / 2,
...
void Machine::DumpRegs(FILE *output, int ptr_x, int ptr_y, unsigned virtual_x,
                       unsigned y, unsigned blocksize) {
  DumpReg(output, registers[PCReg], "PC", "#FF0000", ptr_x, ptr_y, virtual_x, y,
          blocksize);
  DumpReg(output, registers[StackReg], "SP", "#FF0000", ptr_x, ptr_y, virtual_x,
          y, blocksize);
}
...
unsigned Machine::PageTableRoom(unsigned numPages, unsigned blocksize) {
...
static void get_RGB(unsigned char value, unsigned char *r, unsigned char *g,
                    unsigned char *b) {
  *r = (value & 0x7) << 5;
  *g = (value & 0x38) << 2;
...
unsigned Machine::DumpPageTable(FILE *output, TranslationEntry *_pageTable,
                                unsigned _pageTableSize, unsigned addr_x,
                                unsigned virtual_x, unsigned virtual_width,
                                unsigned physical_x, unsigned virtual_y,
                                unsigned y, unsigned blocksize) {
    ...
    if (e->valid) {
      for (offset = 0; offset < PageSize; offset++) {
        ...
        fprintf(output,
                ...
                virtual_y - page * blocksize - blocksize, blocksize, blocksize,
                r, g, b);
      }
      ...
      fprintf(output,
              ...
              virtual_y - page * blocksize - blocksize / 2, physical_x,
  ...
  if (_pageTable == currentPageTable) {
    fprintf(output,
            ...
            "stroke=\"#FF0000\" "
            "stroke-width=\"2\"/>\n",
            virtual_x, virtual_y - _pageTableSize * blocksize, virtual_width,
            _pageTableSize * blocksize);
  }
...
void Machine::DumpMem(const char *name) {
  ...
  fprintf(output,
          ...
          "xmlns:xlink=\"http://www.w3.org/1999/xlink\" viewBox = \"0 0 %u "
          "%u\" version = \"1.1\">\n",
          width, height);
  ...
  fprintf(output,
          ...
          "stroke=\"#000000\" "
          "stroke-width=\"1\"/>\n",
          virtual_x, height - currentPageTableSize * blocksize, virtual_width,
          currentPageTableSize * blocksize);

  DumpAddrSpaces(output, addr_x, ptr_x, virtual_x, virtual_width, physical_x,
                 height, blocksize);
  ...
  for (page = 0; page < NumPhysPages; page++) {
    for (offset = 0; offset < PageSize; offset++) {
      ...
      fprintf(output,
              ...
              height - page * blocksize - blocksize, blocksize, blocksize, r, g,
              b);
    }
  }
