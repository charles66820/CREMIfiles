...
#define PageSize \
  SectorSize  // set the page size equal to
...
class Machine : public dontcopythis {
 public:
  ...
  void DumpReg(FILE *output, int val, const char *name, const char *color,
               int ptr_x, int ptr_y, unsigned virtual_x, unsigned y,
               unsigned blocksize);
  // Dump a register
  ...
  unsigned DumpPageTable(FILE *output, TranslationEntry *pageTable,
                         unsigned pageTableSize, unsigned addr_x,
...
extern void DumpAddrSpaces(FILE *output, unsigned addr_x, unsigned sections_x,
                           unsigned virtual_x, unsigned virtual_width,
