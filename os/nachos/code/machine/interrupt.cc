...
static const char *intTypeNames[] = {"timer",         "disk",
...
Interrupt::~Interrupt() {
  while (!pending->IsEmpty()) delete (PendingInterrupt *)(pending->Remove());
  delete pending;
  pending = NULL;
}
...
void Interrupt::ChangeLevel(IntStatus old, IntStatus now) {
  if (now == IntOff) BlockUserAbort();
  level = now;
  DEBUG('i', "\tinterrupts: %s -> %s\n", intLevelNames[old],
        intLevelNames[now]);
  if (now == IntOn) UnBlockUserAbort();
}
...
IntStatus Interrupt::SetLevel(IntStatus now) {
  ...
  if ((now == IntOn) && (old == IntOff)) OneTick();  // advance simulated time
  return old;
}
...
//	a thread.
//----------------------------------------------------------------------
void Interrupt::Enable() { (void)SetLevel(IntOn); }
...
//----------------------------------------------------------------------
void Interrupt::Schedule(VoidFunctionPtr handler, void *arg, long long fromNow,
                         IntType type) {
...
#ifdef USER_PROGRAM
  if (machine != NULL && status == UserMode) machine->DelayedLoad(0, 0);
#endif
  inHandler = TRUE;
