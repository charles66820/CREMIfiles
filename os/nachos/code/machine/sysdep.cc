...
void Read(int fd, void *buffer, int nBytes) {
...
int ReadPartial(int fd, void *buffer, int nBytes) {
...
void WriteFile(int fd, const void *buffer, int nBytes) {
...
//----------------------------------------------------------------------
void ReadFromSocket(int sockID, void *buffer, int packetSize) {
  int retVal;
  struct sockaddr_un uName;
...
//	Abort on error.
//----------------------------------------------------------------------
void SendToSocket(int sockID, const void *buffer, int packetSize,
                  const char *toName) {
  struct sockaddr_un uName;
  int retVal;
...
void BlockUserAbort(void) { sighold(SIGINT); }
...
void UnBlockUserAbort(void) { sigrelse(SIGINT); }
...
void Abort() {
#ifdef USER_PROGRAM
  if (machine) machine->DumpMem("abort.svg");
#endif
  abort();
}
