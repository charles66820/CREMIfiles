...
MailBox::MailBox() { messages = new SynchList(); }
...
void MailBox::Put(PacketHeader pktHdr, MailHeader mailHdr, const void *data) {
...
void MailBox::Get(PacketHeader *pktHdr, MailHeader *mailHdr, void *data) {
...
PostOffice::PostOffice(NetworkAddress addr, double reliability, int nBoxes) {
  ...
  network =
      new Network(addr, reliability, ReadAvailHandler, WriteDoneHandler, this);
