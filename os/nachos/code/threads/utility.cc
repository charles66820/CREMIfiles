...
// if you have problems with va_start, try both of these alternatives
    ...
    defined(HOST_PPC) || defined(HOST_x86_64)
#include <stdarg.h>
#else
...
//----------------------------------------------------------------------
void SetColor(FILE *output, enum AnsiColor color) {
  if (isatty(fileno(output))) fprintf(output, "\e[%dm", 30 + color);
}
...
//      This assumes that the TTY recognizes ANSI colors
//----------------------------------------------------------------------
void SetBold(FILE *output) {
  if (isatty(fileno(output))) fprintf(output, "\e[1m");
}
...
//      This assumes that the TTY recognizes ANSI colors
//----------------------------------------------------------------------
void ClearColor(FILE *output) {
  if (isatty(fileno(output))) fprintf(output, "\e[0m");
}
