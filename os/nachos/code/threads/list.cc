...
ListElement *List::FirstElement() { return first; }
...
int List::Length(void) {
  ...
  for (cur = first; cur; cur = cur->next) n++;
...
void List::Mapcar(VoidFunctionPtr2 func, void *arg) {
  ListElement *next;
  for (ListElement *ptr = first; ptr != NULL; ptr = next) {
    next = ptr->next;
    DEBUG('l', "In mapcar, about to invoke %p(%p)\n", func, ptr->item);
...
void List::SortedInsert(void *item, long long sortKey) {
