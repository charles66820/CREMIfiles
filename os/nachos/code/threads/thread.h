...
class Thread : public dontcopythis {
 private:
  ...
  unsigned long machineState[MachineStateSize];  // all kernel registers except
                                                 // for stackTop
...
#ifdef USER_PROGRAM
  void DumpThreadState(FILE *output, int ptr_x, int ptr_y, unsigned virtual_x,
                       unsigned virtual_y, unsigned blocksize);
