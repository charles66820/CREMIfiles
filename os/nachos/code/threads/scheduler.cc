...
void Scheduler::Stop() { halted = TRUE; }
...
void Scheduler::Run(Thread *nextThread) {
  ...
  DEBUG('t', "Switching from thread %p \"%s\" to thread %p \"%s\"\n", oldThread,
        oldThread->getName(), nextThread, nextThread->getName());
  ...
  if (threadToBeDestroyed == oldThread)
    __sanitizer_start_switch_fiber(NULL, nextThread->stack,
                                   nextThread->stack_size);
  else
    __sanitizer_start_switch_fiber(&oldThread->fake_stack, nextThread->stack,
                                   nextThread->stack_size);
#endif
