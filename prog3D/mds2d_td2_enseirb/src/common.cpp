...
bool toBool(const std::string &str) {
    ...
    if (value == "false")
        ...
        throw RTException("Could not parse boolean value \"%s\"", str);
}
...
int toInt(const std::string &str) {
    ...
    if (*end_ptr != '\0')
        throw RTException("Could not parse integer value \"%s\"", str);
    return result;
}
...
float toFloat(const std::string &str) {
    ...
    if (*end_ptr != '\0')
        throw RTException("Could not parse floating point value \"%s\"", str);
    return result;
}
...
Eigen::Vector3f toVector3f(const std::string &str) {
    ...
    if (tokens.size() != 3)
        throw RTException("Expected 3 values");
...
Eigen::Quaternionf toQuaternionf(const std::string &str) {
    std::vector<std::string> tokens = tokenize(str);
    if (tokens.size() != 4)
        throw RTException("Expected 4 values");
    Eigen::Vector4f result;
    for (int i=0; i<4; ++i)
        result[i] = toFloat(tokens[i]);
    return Eigen::Quaternionf(result).normalized();
}
...
Eigen::Vector2f sphericalCoordinates(const Eigen::Vector3f &v) {
