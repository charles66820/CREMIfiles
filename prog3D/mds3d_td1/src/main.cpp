...
void render(Scene* scene, ImageBlock* result, std::string outputName, bool* done)
{
    ...
    Vector3f camX =
        camera->right() * tanfovy2 * camera->nearDist() * float(camera->vpWidth()) / float(camera->vpHeight());
    ...
    for (uint i = 0; i < width; i++)
        for (uint j = 0; j < height; j++) {
            Vector3f p;
            p = (camF * camF.norm()) + 2 * (i / (width - 0.5)) * camX + 2 * (j / (height - 0.5)) * camY;

            Vector3f direction = (p - C).normalized();
